...
def segment_signal(data, window_size, overlap):
    ...
    labels = np.append(labels, stats.mode(data['label'][0:300])[0])
    count = 0
    for (start, end) in windows(data['acc_x'], window_size, overlap):
        count += 1
        ...
        if len(data['acc_x'][start:end]) == window_size:
